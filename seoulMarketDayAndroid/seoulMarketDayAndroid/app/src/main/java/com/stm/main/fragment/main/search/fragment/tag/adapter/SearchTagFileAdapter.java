...
public class SearchTagFileAdapter  extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        final SearchTagFileViewHolder searchTagFileViewHolder = new SearchTagFileViewHolder(searchTagPresenter, file, view);
        view.setTag(searchTagFileViewHolder);

        if (type == DefaultFileFlag.PHOTO_TYPE) {
            String uri = searchTagFileViewHolder.storyImageUrl + file.getName() + "." + file.getExt();

            searchTagFileViewHolder.ll_searchtag_photo.setVisibility(View.VISIBLE);
            searchTagFileViewHolder.ll_searchtag_video.setVisibility(View.GONE);
            searchTagFileViewHolder.ll_searchtag_vr.setVisibility(View.GONE);

            searchTagFileViewHolder.ll_searchtag_photo.bringToFront();
            Glide.with(context).load(uri).thumbnail(0.5f).diskCacheStrategy(DiskCacheStrategy.ALL).into(searchTagFileViewHolder.iv_searchtag_photo);
        }

        if (type == DefaultFileFlag.VIDEO_THUMBNAIL_TYPE) {
            String uri = searchTagFileViewHolder.storyVideoUrl + file.getName() + "." + file.getExt();

            searchTagFileViewHolder.ll_searchtag_photo.setVisibility(View.GONE);
            searchTagFileViewHolder.ll_searchtag_video.setVisibility(View.VISIBLE);
            searchTagFileViewHolder.ll_searchtag_vr.setVisibility(View.GONE);

            searchTagFileViewHolder.ll_searchtag_video.bringToFront();
            searchTagFileViewHolder.iv_searchtag_videoplayer.bringToFront();
            Glide.with(context).load(uri).thumbnail(0.5f).diskCacheStrategy(DiskCacheStrategy.ALL).into(searchTagFileViewHolder.iv_searchtag_video);
        }

        if (type == DefaultFileFlag.VR360_TYPE) {
            String uri = searchTagFileViewHolder.storyVr360Url + file.getName() + "." + file.getExt();
            searchTagFileViewHolder.ll_searchtag_photo.setVisibility(View.GONE);
            searchTagFileViewHolder.ll_searchtag_video.setVisibility(View.GONE);
            searchTagFileViewHolder.ll_searchtag_vr.setVisibility(View.VISIBLE);

            searchTagFileViewHolder.ll_searchtag_vr.bringToFront();
            Glide.with(context).load(uri).asBitmap().diskCacheStrategy(DiskCacheStrategy.ALL).into(new SimpleTarget<Bitmap>() {
                ...
                public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                    ...
                    searchTagFileViewHolder.vrpv_searchtag_vr.setInfoButtonEnabled(false);
                    searchTagFileViewHolder.vrpv_searchtag_vr.loadImageFromBitmap(resource, options);
                }
            });
        }
